...
const CLASS_MN: { [key: string]: string } = {
  "Hardhat": "Каск",
  "Mask": "Амны хаалт",
  "NO-Hardhat": "Каскгүй",
  "NO-Mask": "Амны хаалтгүй",
  "NO-Safety Vest": "Аюулгүйн хантаазгүй",
  "Person": "Хүн",
  "Safety Cone": "Аюулгүйн конус",
  "Safety Vest": "Аюулгүйн хантааз",
  "machinery": "Техник хэрэгсэл",
  "vehicle": "Тээврийн хэрэгсэл"
};
const MAIN_PPE_CLASSES = ["Hardhat", "Mask", "Safety Vest"];
type Detection = {
  class_name: string;
  score: number;
  bbox: number[]; // эсвэл [number, number, number, number]
};
const getMissingPPE = (detections: Detection[]): string[] => {
  const foundClasses = detections ? detections.map(d => d.class_name) : [];
  return MAIN_PPE_CLASSES.filter(c => !foundClasses.includes(c));
};
...
console.log(res.data.detections); // {detections: [ ... ]}
  ...
  if (result.detections && result.detections.length > 0) {
    // Илрээгүй хамгаалах хэрэгслүүдийг авна
    const missingPPE = getMissingPPE(result.detections);
    ...
    return (
      <Animated.View style={{ opacity: fadeAnim, alignItems: 'center', marginTop: 12 }}>
        ...
        <Text style={[styles.ok, { marginBottom: 10 }]}>
          Илэрсэн зүйлс:
          <Text style={{ color: "#2F487F", fontWeight: "600" }}> {result.detections.length} </Text>
        </Text>
  ...
  <View key={i} style={styles.detectionItem}>
    {/* ... */}
    <Text>
      {CLASS_MN[det.class_name] || det.class_name}
    </Text>
    {/* ... */}
  </View>
))}
        ...
        {missingPPE.length > 0 && (
          <View style={{
            ...
            <Ionicons name="close-circle" size={22} color="#EF5350" style={{ marginBottom: 3 }} />
            <Text style={{
              color: "#EF5350",
              fontSize: 16,
              fontWeight: "600",
              ...
              Илрээгүй хамгаалах хэрэгсэл:
            </Text>
            {missingPPE.map((mn, idx) => (
              <Text key={idx} style={{ color: "#EF5350", fontSize: 15, marginTop: 2 }}>
                • {CLASS_MN[mn] || mn}
              </Text>
            ))}
          </View>
  ...
  return (
    <Animated.View style={{ opacity: fadeAnim, alignItems: 'center', marginTop: 12 }}>
      ...
      <Text style={styles.notOk}>Ямар нэгэн хамгаалах хэрэгсэл илрээгүй байна!</Text>
    </Animated.View>
  );
};
  ...
  return (
    <View style={styles.outerContainer}>
      <Header />
<Text style={styles.title}>Аюулгүй ажиллагааны хамгаалах хэрэгслийн шалгалт</Text>
